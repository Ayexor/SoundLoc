...
This is describet in section \ref{sec::cic}.
...
Finaly a post divider is implemented to scale the decimated data by powers of $\sfrac{1}{2}$.
This can be used to eliminate problems caused by overflow in the following cross correlation that may occure with high order and decimation rate.
The post divider is controlled by software and describet in section \ref{sec::post_divider}.
...
Each can be acessed directly by their address or more confeniently by the software driver described in section \ref{sec::driver}.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|l|l|}
		...
		\hline 
		Status & 0x00 & R/W & Contains Statuzs and controll bits discribed in table \ref{tbl::status_register} \\
		\hline 
		Decimation & 0x04 & R/W & Contains the decimation rate of the CIC filter \\
		\hline 
		Value0 & 0x08 & R & Contains the actual, filtered value for microphon 2 \\
		\hline 
		Value1 & 0x0C & R & Contains the actual, filtered value for microphon 1 \\
		\hline 
		Value2 & 0x10 & R & Contains the actual, filtered value for microphon 3 \\
		\hline 
	\end{tabular} 
\end{table}
...
The microphones apply the new bits on fallign edge and the Filter reads them on rising edge.
...
A CIC filter is fully charactericed by its order, the differential delay and the decimation rate.
The resulting transferfunction is shown in equation \ref{eq::cic_tf}.
...
The decimation is limited to the internal register width defined by paramter D\_WIDTH.
...
With high decimation rates and orders, overflow can occure.
...
\label{sec::iir}
A simple IIR DC blocker is available to remove DC components due to microphon offsets.
This may be needed as the correlation accumulats this DC component and may overflow.
The filter has the transfer function given in equation \ref{eq::iir_tf}.
The pole is set close to $z=1$ to optain a low cutoff frequency.
...
The post divider can divide the internaly calculated values by powers of two.
The high gain from the CIC filter can therefor be scaled.
This is handy if the internal width is choosen greater than the output width.
...
\begin{itemize}
	\item call SDM\_DECIM\_setStatus(baseaddr, mask) to initialize the IP core. \\
		This sets the conten of the status register to mask. See section \ref{sec::registers} for further information.\\
	...
	\item If the raw values are of interest, call SDM\_DECIM\_getValue(baseaddr, mic).\\
		This returns a 32 bit signed integer that corresponds to the microphon mic. \\
...
On the PCB thei are nuberd from 1 to 3 but in hardware the numbering goes from 0 to 2.
