...
//	if (isnan(angle)) std::cout << "LedDisplay_Angle: angle is NaN" << std::endl;
//	else if(angle > 360.0) std::cout << "LedDisplay_Angle: angle is too big" << std::endl;
//	else if(angle >= (360.0-1*halfstep)) val = (1<<7);
	if(angle >= (360.0-1*halfstep)) val = (1<<7);
...
//	double angle = atan2(tau12, tau13)*180/M_PI + 90;
//	angle += 15*sin(2*M_PI/180*angle) + 2.2*sin(4*M_PI/180*angle);
	//std::cout << "LedDisplay_Coordinate: angle is " << angle << std::endl;
//	std::cout << tau12; std::cout << "," << tau13 << std::endl;

	double x, y, angle;

	x = tau12 - M_SQRT1_2 * tau13;
	y = M_SQRT2 * tau13;


	angle = atan2(y,x)*180.0/M_PI + 180.0;
	LedDisplay_Angle(angle);
}
